...
/**
 * Marks a no-target boost as ready.
 *
 * Better Price is the only one today: it has nothing to point at, because the
 * next bet placed consumes it. `used_at` stays NULL on purpose -- consumeOddsBoost
 * looks for exactly that, so setting it here would make the boost unusable.
 * What this really does is confirm the player meant to arm it.
 */
export async function armBoost({ slug, boostId }) {
  const [boost] = await sql`
    select id, owner, kind, used_at, detail from boosts where id = ${boostId}`;
  if (!boost) throw new Error('No such boost.');
  if (boost.owner !== slug) throw new Error('That is not yours.');
  if (boost.used_at) throw new Error('That boost has already been used.');
  ...
  const def = byKind[boost.kind];
  if (!def) throw new Error('No such boost.');
  if (def.target !== TARGET.NEXT_BET) {
    throw new Error(`${def.name} needs something to use it on.`);
  }
  if (boost.detail?.armed) throw new Error(`${def.name} is already armed.`);

  const [row] = await sql`
    update boosts set detail = detail || '{"armed": true}'::jsonb
    where id = ${boostId} and used_at is null
    returning id, kind, detail`;
  if (!row) throw new Error('That boost has already been used.');
  return row;
 ...
 *
 * Two kinds behave this way and neither has a visible target, so without this
 * they were invisible: Better Price is consumed by the very next bet placed,
 * and Big Week is already applied to the whole week. Someone could arm Better
 * Price and then spend it on a $10 punt without realising it was live.
 */
export async function getArmedBoosts(slug, season, week) {
  const rows = await sql`
    ...
    where owner = ${slug} and season = ${season}
      and (
        (kind = 'odds-boost' and used_at is null and detail->>'armed' = 'true')
        or (kind = 'boost-week' and used_at is not null and (detail->>'week')::int = ${week})
      )`;
  return rows.map((r) => {
    ...
    return {
      ...
      note:
        r.kind === 'odds-boost'
          ? 'fires on your next bet'
          : `is live for week ${week}`,
    };
  });
}
