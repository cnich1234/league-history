...
async function loadProjections(season, week) {
  ...
  return { projections: byId, teams: teamById, positions: positionById };
}
...
export async function buildWeek(sql, season, week) {
  ...
  const [{ projections, teams: nflTeams, positions: feedPositions }, gameDates, slim] = await Promise.all([
