...
export async function kickedOffTeams(season, week) {
  ...
  for (const [team, game] of Object.entries(gameByTeam)) {
    const m = game.metadata ?? {};
    if (game.status === 'complete' || m.is_over || m.closed) out.add(team);
    else if (m.has_started || m.is_in_progress || m.has1st_quarter_started) out.add(team);
  }
  return out;
}
