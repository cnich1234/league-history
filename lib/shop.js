 ...
 */
export async function useBoostOnBet({
  slug,
  boostId,
  betId,
  season = null,
  phaseOf = null,
  // True only from the bet and parlay routes, in the same request that
  // placed the bet. The slip-only boosts are refused without it.
  atPlacement = false,
}) {
  ...
  // Insurance, Half Again and Mirror are called when the bet is placed, not
  // bolted on later once a bounty has appeared or the bet is going well.
  if (def.slipOnly && !atPlacement) {
    throw new Error(`${def.name} goes on when you place the bet, not afterwards.`);
  }
  ...
  // An attack on a finished game is not a gamble. Until the phase existed an
  // attack was allowed on any pending bet -- a Grand Theft on a known winner,
  // any time between the final whistle and Tuesday's settlement.
  if (def.attack) {
    const phase = await (phaseOf ?? (await import('./book.js')).phaseOfBet)(betId);
    if (phase === 'closed') {
      throw new Error('That game is over. A bet whose result is in cannot be attacked.');
    }
  }
  ...
  if (def.attack) {
    ...
    if (mirrored[betId]) {
      const own = await biggestOpenBet(slug, targetSeason, { unhit: true });
      if (!own) {
        throw new Error(
...
export async function slipBoosts(slug, season) {
  const rows = await sql`
    ...
    where owner = ${slug} and season = ${season} and used_at is null
      and kind in ('odds-boost', 'insurance', 'boost-50', 'mirror', 'lock-in')
 ...
 */
export async function rideAlong({ slug, boostId, betId, phaseOf = null }) {
  const [boost] = await sql`
    select id, owner, kind, used_at from boosts where id = ${boostId}`;
  if (!boost) throw new Error('No such boost.');
  if (boost.owner !== slug) throw new Error('That is not yours.');
  ...
  // Before kickoff only. Once the game is underway a copy is made with
  // hindsight -- and on a live market it would be struck at the ORIGINAL
  // price, which the market no longer offers.
  const phase = await (phaseOf ?? (await import('./book.js')).phaseOfBet)(betId);
  if (phase !== 'open') {
    throw new Error('Ride Along only works before the games start -- that one is underway.');
  }
...
export async function undoBet({ slug, boostId, betId }) {
  ...
  // A bounty still raising for this bet can never land now. Its backers get
  // their points back today rather than when the cron notices on Tuesday.
...
export async function readReceipt({ slug, boostId, betId }) {
  ...
  const rows = await sql`
    select b.kind, t.display_name as who, b.used_at, b.detail->>'bounty' as bounty_id
    from boosts b
    join bettors t on t.slug = b.owner
    where b.target_bet_id = ${betId} and b.owner <> ${slug}
    order by b.used_at`;

  // A crowd-funded hit is owned by its poster on the row, but it names
  // everyone who paid for it. One entry per backer, with what they put in --
  // chipping in is not anonymous.
  const attackers = [];
  for (const r of rows) {
    if (!r.bounty_id) {
      attackers.push({ kind: r.kind, who: r.who });
      continue;
    }
    for (const k of await bountyBackers(Number(r.bounty_id))) {
      attackers.push({ kind: r.kind, who: k.display_name, points: k.points, bounty: true });
    }
  }
  ...
  return attackers.map((a) => ({
    ...
    who: a.who,
    ...(a.bounty ? { bounty: true, points: a.points } : {}),
  }));
}
 ...
 * the attack to.
 */
export async function biggestOpenBet(slug, season, { unhit = false } = {}) {
  const [row] = await sql`
    select b.id
    from bets b
    left join markets m on m.id = b.market_id
    where b.bettor = ${slug} and b.status = 'pending'
      -- A straight bet carries its season through its market; a parlay has no
      -- market of its own and carries it through its legs. Letting a parlay
      -- through unchecked meant a rebound could land on one from another
      -- season entirely, which the sentinel-season suites found first.
      and (
        (b.market_id is not null and m.season = ${season})
        or (b.market_id is null and exists (
          select 1 from parlay_legs l join markets lm on lm.id = l.market_id
          where l.bet_id = b.id and lm.season = ${season}))
      )
      -- A rebound has to land on a bet that can still take an attack.
      and (${!unhit} or not exists (
        select 1 from boosts a
        where a.target_bet_id = b.id and a.kind = any(${ATTACK_KINDS})))
...
export async function cashOut({ slug, boostId, betId, priceNow = quoteNow }) {
  ...
  // Attacks stick. Cashing out paid the OWNER the full value of a bet that
  // had been stolen, voided or skimmed -- so whoever paid for the attack,
  // alone or as a crowd, got nothing, and the victim walked off with the
  // money. A hit bet rides to settlement, where the attack is applied.
  const [hit] = await sql`
    select kind from boosts where target_bet_id = ${betId} and kind = any(${ATTACK_KINDS})`;
  if (hit) {
    throw new Error(
      `That bet has been hit with ${byKind[hit.kind]?.name ?? hit.kind} -- it rides to settlement.`,
    );
  }
 ...
 */
export async function switcheroo({ slug, boostId, betId, phaseOf = null }) {
  const [boost] = await sql`
    select id, owner, kind, used_at from boosts where id = ${boostId}`;
  if (!boost) throw new Error('No such boost.');
  if (boost.owner !== slug) throw new Error('That is not yours.');
  ...
  if (bet.status !== 'pending') throw new Error('That bet has already settled.');
  if ((await (phaseOf ?? (await import('./book.js')).phaseOfBet)(betId)) === 'closed') {
    throw new Error('That game is over. A bet whose result is in cannot be attacked.');
  }
  ...
  let reflected = false;
  if (mirrored[betId]) {
    const own = await biggestOpenBet(slug, season, { unhit: true });
    if (!own) {
      throw new Error(
 ...
 */
export async function postBounty({ slug, season, week, target, weapon, betId, points, phaseOf = null }) {
  ...
  if (needsBet) {
    ...
    if ((await (phaseOf ?? (await import('./book.js')).phaseOfBet)(betId)) === 'closed') {
      throw new Error('That game is over. Nothing can be done to that bet now.');
    }
  ...
  const state = await contributeToBounty({
    slug, season, bountyId: Number(row.id), points: seed, phaseOf,
  });
 ...
 */
export async function contributeToBounty({ slug, season, bountyId, points, phaseOf = null }) {
  ...
  let fired = null;
  if (now >= cost) fired = await fireBounty(bountyId, { phaseOf });
 ...
 */
export async function fireBounty(bountyId, { phaseOf = null } = {}) {
  ...
  if (b.bet_id != null) {
    ...
    // And a finished game takes no attack, crowd-funded or not.
    const phase = await (phaseOf ?? (await import('./book.js')).phaseOfBet)(b.bet_id);
    if (phase === 'closed') {
      await refundBounty(bountyId, 'the game was over before it filled');
      return { refunded: true, why: 'the game was over before it filled' };
    }
  }

  // A Mirror sends a bought attack back at its attacker. A crowd-funded one
  // rebounds onto EVERY backer -- each took part, so each takes the hit on
  // their own biggest open bet.
  if (b.bet_id != null) {
    const mirrored = await mirroredBets([Number(b.bet_id)]);
    if (mirrored[b.bet_id]) return reflectBounty(b, def, bountyId);
  }
  ...
  let moved = null;
  if (def.flips) moved = await moveBet(b.bet_id);
  ...
  const [boost] = await sql`
    ...
    values (${b.poster}, ${b.season}, ${b.weapon}, ${Number(b.cost_points)},
            ${b.bet_id}, ${b.bet_id == null ? b.target : null}, now(),
            ${JSON.stringify({
  ...
  return { fired: true, boostId: String(boost.id), weapon: def.name, reflected: false };
}

/**
 * A mirrored bet turns a bounty back on the people who funded it.
 *
 * Each backer takes the named attack on their own biggest open bet that can
 * still take one. The row is theirs -- owner and target both -- and carries no
 * `bounty` marker, or a Grand Theft's payout would be split among the crowd
 * for stealing from one of their own. A backer with nothing to rebound onto
 * gets their contribution back, exactly as a bought attack is refused rather
 * than wasted when its attacker has no open bet.
 */
async function reflectBounty(b, def, bountyId) {
  const backers = await bountyBackers(bountyId);
  const landed = [];
  const refunded = [];
  for (const k of backers) {
    const own = await biggestOpenBet(k.contributor, Number(b.season), { unhit: true });
    if (!own) {
      await sql`
        insert into point_ledger (bettor, season, amount, reason, note)
        values (${k.contributor}, ${b.season}, ${k.points}, 'refund',
                ${'Bounty mirrored -- nothing of yours to rebound onto'})`;
      refunded.push(k.contributor);
      continue;
    }
    const moved = def.flips ? await moveBet(own) : null;
    await sql`
      insert into boosts (owner, season, kind, cost_points, target_bet_id, used_at, detail)
      values (${k.contributor}, ${b.season}, ${b.weapon}, ${k.points}, ${own}, now(),
              ${JSON.stringify({
                reflectedFrom: String(bountyId),
                week: Number(b.week),
                reflected: true,
                aimedAt: String(b.bet_id),
                ...(moved?.detail ?? {}),
              })}::jsonb)`;
    landed.push({ slug: k.contributor, betId: String(own) });
  }
  await sql`
    update bounties set status = 'fired', fired_at = now(), closed_reason = 'mirrored'
    where id = ${bountyId}`;
  return { fired: true, reflected: true, weapon: def.name, landedOn: landed, refunded };
}
