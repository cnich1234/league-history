 ...
 *
 * The all-chalk lineup must be UNAFFORDABLE, or there is no choice to make: at
 * these numbers it costs about $92,800 against a $50,000 cap, roughly double.
 *
 * The cheapest legal lineup must leave room to SPEND. A floor of $3,000 charged
 * real money for replacement-level players, so simply fielding nine bodies ate
 * 73% of the cap and left $13,600 -- about one good quarterback -- to upgrade
 * anybody. Dropping the floor to $2,000 and steepening the curve to $500 a
 * point puts the minimum lineup at 51% of the cap while leaving chalk exactly
 * as unaffordable.
 */
export const SALARY_FLOOR = 2000;
export const SALARY_PER_POINT = 500;
...
export async function fetchProjections(season, week) {
  ...
  for (const r of Array.isArray(rows) ? rows : []) {
    ...
    projections[String(id)] = Number(r.stats?.pts_half_ppr ?? r.stats?.pts_ppr ?? 0);
  }
