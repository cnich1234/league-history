...
export async function placeBet({
  ...
  // A market already in one of your live parlays cannot also be bet straight.
  // The slip enforced this and the server did not, so a direct POST -- or a
  // stale tab -- could pay for a single and a leg on the same game.
  const [inParlay] = await sql`
    select 1 from parlay_legs l join bets b on b.id = l.bet_id
    where b.bettor = ${slug} and b.status = 'pending' and l.market_id = ${marketId}`;
  if (inParlay) throw new Error('That market is already in one of your parlays.');
...
export async function placeParlay({ slug, legs, stakeCents }) {
  ...
  // The other direction of the same rule: no leg on a market you have bet
  // straight, whether that bet is still open or already decided.
  const [straight] = await sql`
    select m.title from bets b join markets m on m.id = b.market_id
    where b.bettor = ${slug} and b.market_id = any(${marketIds}) limit 1`;
  if (straight) throw new Error(`You already have a straight bet on ${straight.title}.`);
