...
export function groupByMatchup(markets, allMarkets = markets) {
  // Pairings come from EVERY h2h in the week, not just the open ones. A
  // matchup locks before its props do, so building the list from open h2h
  // markets alone left later-locking props with no game to attach to and they
  // were silently dropped -- five of ten props vanished from the board once
  // three matchups had locked.
  const games = [];
  const seen = new Set();
  ...
  // Only the markets actually passed in get attached, so a caller asking for
  // open markets gets open ones even though the pairings came from all of them.
  for (const m of markets) {
  ...
  for (const g of games) {
    ...
    // The group locks when its earliest REMAINING market does, so a card whose
    // matchup has closed still sorts by whatever is left in it.
    if (all.length) {
  ...
  // Drop games with nothing left to show; the caller decides what to do with
  // an empty week.
  return games
    .filter((g) => g.marketCount > 0)
    .sort((a, b) => new Date(a.locksAt) - new Date(b.locksAt));
}
