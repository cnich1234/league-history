...
async function loadProjections(season, week) {
  ...
  return { projections: byId, teams: teamById, positions: positionById, injuries: injuryById };
}
...
export async function buildWeek(sql, season, week) {
  ...
  const [
    { projections, teams: nflTeams, positions: feedPositions, injuries: feedInjuries },
    gameDates,
    slim,
  ] = await Promise.all([
  ...
  const lineupEntries = (m) =>
    expectedLineup(m, {
      ...
      doubtful: (id) => isRisky(feedInjuries?.[id]),
      replacement,
    });
