...
import { recordTick, seriesFor, trackFor, pricesAt, TICK_EVERY_MS, QUIET_EVERY_MS } from './ticks.js';
import { premiums, weekStart } from './baselines.js';
...
export async function explain({ ticker, range = DEFAULT_RANGE, now = Date.now() }) {
  ...
  // Same window as the chart, anchored to the roll for WEEK. When these two
  // disagreed, a move could be visible on the chart and missing from the list
  // underneath it -- or, as happened on 2026-09-16, gone from both while still
  // sitting in the log, which reads as data loss rather than a narrow window.
  const rolledAt = await weekStart(board.season, board.week).catch(() => null);
  const { from } = rangeWindow(rng, now, rolledAt);
  const track = await trackFor(player.id, from, now);
  return { player, range: rng, weekStart: rolledAt, track, moves: explainTrack(track) };
}
...
export async function quotes({ source = 'live', owner = null, now = Date.now() } = {}) {
  ...
  if (src === 'live') {
    ...
    // The watchlist's change is measured from the WEEK'S OPEN, the same
    // reference the stock page uses. It used to come off basePrice, which is
    // computed from the current projection -- so before kickoff every row on
    // the board read 0.00 and 0.00%, including players whose price had halved
    // since Tuesday. One query for the whole board, not one per row.
    const rolledAt = await weekStart(board.season, board.week).catch(() => null);
    const [opens, balance] = await Promise.all([
      rolledAt ? pricesAt(rolledAt).catch(() => ({})) : Promise.resolve({}),
      owner ? pointsFor(owner, board.season).catch(() => null) : Promise.resolve(null),
    ]);
    const rows = board.rows.map((r) => {
      const open = Number(opens[String(r.id)]);
      const hasOpen = Number.isFinite(open) && open > 0;
      return {
        ...r,
        owned: owned[String(r.id)] ?? 0,
        weekOpen: hasOpen ? r2(open) : null,
        change: hasOpen ? r2(r.price - open) : r.change,
        changePct: hasOpen ? r2((r.price / open - 1) * 100) : r.changePct,
      };
    });
    return { source: src, asOf: now, season: board.season, week: board.week, rows, balance };
  }
  const { season, week } = await nflState();
...
export async function history({ source = 'live', ticker, range = DEFAULT_RANGE, owner = null, now = Date.now() }) {
  ...
  if (src === 'live') {
    ...
    // The roll is what anchors the WEEK range. Null in week 1, where it falls
    // back to a rolling seven days.
    const rolledAt = await weekStart(board.season, board.week).catch(() => null);
    const { from, to, step } = rangeWindow(rng, now, rolledAt);
    const owned = await ownedBy(owner, board.season);
    const player = {
      ...found,
    ...
    const inRange = series.filter((s) => s.t >= from);
    const candles = toCandles([...inRange, { t: now, price: player.price }], step);

    // High, low and the reference price all follow the SELECTED RANGE rather
    // than a hardcoded 24 hours. On a quiet Wednesday the old version showed
    // "24h high 1.98, 24h low 1.98" beside a price that had fallen from 8.75
    // on Tuesday -- true, and useless.
    const prices = inRange.map((s) => s.price);
    const rangeHigh = r2(Math.max(player.price, ...(prices.length ? prices : [player.price])));
    const rangeLow = r2(Math.min(player.price, ...(prices.length ? prices : [player.price])));

    // Where the range opened, which is what a change is measured against. For
    // WEEK that is the price at the roll, so "down 6.77 since the week began"
    // is a sentence the page can finally say. basePrice(projection, premium)
    // moves with the projection, so it could never be that reference: it read
    // 0.00 and 0.00% no matter what had happened.
    const openPrice = inRange.length ? inRange[0].price : player.prevClose;
    const change = r2(player.price - openPrice);
    ...
    return {
      ...
      player: {
        ...player,
        rangeOpen: r2(openPrice),
        rangeHigh,
        rangeLow,
        change,
        changePct: openPrice ? r2((player.price / openPrice - 1) * 100) : 0,
