...
/**
 * The tradable list: the top of the projections, plus anyone in `include` --
 * a player somebody holds keeps a price even after he drops out of the top.
 * The full ticker assignment is cached; the cut is applied per call.
 */
export async function loadUniverse({ season, week, include = [] } = {}) {
  const all = await loadAll({ season, week });
  const wanted = new Set(include.map(String));
  return all.filter((p, i) => i < UNIVERSE_SIZE || wanted.has(String(p.id)));
}
