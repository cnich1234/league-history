...
export function groupByMatchup(markets, allMarkets = markets) {
  ...
  // markets alone left later-locking props with no game to attach to.
  const pairings = [];
  const seen = new Set();
  ...
  // One card per (matchup, lock day). A matchup whose spread closes Wednesday
  // and whose props close Saturday produces two cards, each holding only the
  // bets that actually close then -- otherwise a Sunday prop appears under a
  // Thursday heading purely because it shares a game with a Thursday market.
  const byKey = new Map();
  for (const m of markets) {
    ...
    const day = new Date(m.locks_at).toISOString().slice(0, 10);
    const key = `${pairing.key}@${day}`;
    if (!byKey.has(key)) {
      byKey.set(key, {
        key,
  ...
  for (const g of games) g.marketCount = Object.values(g.markets).flat().length;
