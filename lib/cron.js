...
export async function buildWeek(sql, season, week) {
  ...
  for (const pair of Object.values(byMatchup)) {
    ...
    // Alternate ("blowout") spreads. Same kind, same resolver, same live model
    // as the main spread -- only the line and the price differ. Priced off the
    // model rather than picked by hand, so a lopsided matchup does not hand out
    // +320 on something that is actually likely.
    for (const extra of [20.5, 30.5, 40.5]) {
      // Only worth offering beyond the main line; below it the main spread
      // already covers that question at a better price.
      if (extra <= abs) continue;
      const pCover = h2hProbability(
        favourite === home ? projHome : projAway,
        (favourite === home ? projAway : projHome) + extra,
      );
      // Nothing anyone would take, and nothing worth the row on a phone.
      if (pCover < 0.04) continue;
      const priced = twoWayOdds(pCover);
      await createMarket({
        kind: 'spread',
        title: `${favourite.team} -${extra} vs ${underdog.team}`,
        subtitle: `Blowout: does ${favourite.team} win by more than ${extra}?`,
        meta: {
          homeRoster: a.roster_id,
          awayRoster: b.roster_id,
          homeSlug: home.slug,
        ...
        locksAt: matchupLock,
        options: [
          { key: 'cover', label: `${favourite.team} -${extra}`, odds: priced.home },
          { key: 'nocover', label: `${underdog.team} +${extra}`, odds: priced.away },
        ],
      });
    }
