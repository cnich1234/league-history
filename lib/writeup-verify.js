...
export function checkNumbers(markdown, context) {
  ...
  // The side games. A recap that covers betting, daily fantasy and trophies
  // quotes these constantly -- a DFS score, a player's salary, what somebody
  // is up on the season -- and without them every one reads as invented.
  //
  // RAW VALUES ONLY, no sums or differences. Whitelisting derived combinations
  // was tried on the football numbers and it destroyed the gate: the whitelist
  // grew big enough that 83% of randomly invented figures matched something by
  // coincidence, against 47% before. These 109 raw values cost about five
  // points of coverage, which is the honest price of being able to write about
  // the half of the league that is not football. Arithmetic on them is left to
  // the agents, who can check it against named players.
  const games = context.games;
  if (games) {
    for (const e of games.daily?.field ?? []) {
      add(e.points);
      add(e.salaryUsed);
      for (const pl of e.lineup ?? []) {
        add(pl.points);
        add(pl.salary);
      }
    }
    for (const b of games.betting?.seasonToDate ?? []) {
      add(b.netDollars);
      // Written as "down $105.08" as often as "-105.08".
      add(Math.abs(Number(b.netDollars ?? 0)));
      add(b.wageredDollars);
    }
    for (const t of games.trophies ?? []) add(t.points);
    for (const byReason of Object.values(games.pointsEarned?.byManager ?? {})) {
      for (const n of Object.values(byReason)) add(n);
    }
  }
  ...
  for (const [i, v] of agentVerdicts.entries()) {
    if (!v || typeof v.pass !== 'boolean' || !Array.isArray(v.claims)) {
      agentProblems.push({
        ...
        broken: true,
        why:
          `Verification ${i + 1} returned no usable verdict. This is a broken ` +
          'verifier, not a finding about the draft -- check that the headless ' +
          'CLI is still authenticated (claude --print should answer) and re-run.',
      });
      continue;
    }
    // A verifier that checked nothing is not a verifier that found nothing.
    //
    // On 2026-09-23 the headless CLI OAuth session expired mid-run. Agent 1
    // could not reach Sleeper, the database or data/league.json, so it marked
    // all 44 claims UNVERIFIABLE and returned pass=false; agent 2 returned
    // nothing parseable. The run blocked -- correctly -- but reported it as
    // "failed without naming a claim", which reads exactly like a verifier
    // that ran fine and disagreed about something it could not articulate.
    //
    // That ambiguity is the hazard. The temptation on a blocked run is to
    // skim the problems, recognise the familiar false positives and publish
    // anyway; if a broken verifier looks the same as a working one, that
    // habit eventually waves through a real error. So say which it was.
    const checked = v.claims.filter((c) => c.verdict !== 'UNVERIFIABLE');
    if (v.claims.length > 0 && checked.length === 0) {
      agentProblems.push({
        severity: 'error',
        broken: true,
        why:
          `Verification ${i + 1} could not check anything: all ${v.claims.length} ` +
          'claims came back UNVERIFIABLE. This is a broken verifier, not a ' +
          'finding about the draft -- check that the headless CLI is still ' +
          'authenticated (claude --print should answer) and re-run.',
      });
      continue;
    }
