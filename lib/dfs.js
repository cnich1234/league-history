...
/**
 * The lineup: eight slots.
 *
 * Started as the league's own Sleeper roster -- three WRs and a kicker, ten
 * slots, ten of eleven positions filled before any choice was made. Dropped
 * to two WRs to match what daily fantasy looks like, then dropped the kicker:
 * kickers priced into a band so narrow they were a fixed cost rather than a
 * decision, and a slot with no decision in it is a tax on the cap. FLEX still
 * takes RB, WR or TE, so a third receiver is a choice rather than a rule.
 */
export const LINEUP = ['QB', 'RB', 'RB', 'WR', 'WR', 'TE', 'FLEX', 'DEF'];
export const FLEX_POSITIONS = ['RB', 'WR', 'TE'];

/** Positions that get a price. No kickers: nothing in the lineup takes one. */
export const DFS_POSITIONS = ['QB', 'RB', 'WR', 'TE', 'DEF'];

/**
 * Fits a stored lineup to the current shape.
 *
 * The lineup was nine slots with a kicker at index 7. A draft or entry saved
 * under that shape has its kicker dropped and everything else kept in place;
 * anything else of the wrong length is padded or cut to fit, so a stale shape
 * can never make the builder or the validator throw.
 */
export function fitSlots(slots) {
  let list = Array.isArray(slots) ? [...slots] : [];
  if (list.length === LINEUP.length + 1) list.splice(7, 1); // the old K slot
  while (list.length < LINEUP.length) list.push(null);
  return list.slice(0, LINEUP.length);
}
 ...
 */
export async function enterContest({ slug, contestId, slots }) {
  ...
  // The CONTEST's week, never one the client names: validating against a
  // different week's salaries would price a lineup off the wrong board.
  const pool = await salaryPool(contest.season, contest.week);
  const check = validateLineup(slots, pool);
  if (!check.ok) throw new Error(check.why);

  // PER-PLAYER locking, genuinely per player. A slot whose man has already
  // taken the field is frozen; every other slot stays editable for as long as
  // its own player has not.
  //
  // It used to freeze the WHOLE lineup the moment any one player started, so a
  // Thursday quarterback locked the seven Sunday slots behind him for three
  // days -- for no reason, since nothing about those players was known yet.
  //
  // Both directions matter. You cannot swap a started player OUT (his points
  // are already on the board) and you cannot swap one IN (his game began
  // without him being yours). Everything else is fair game.
  ...
  if (locked.size) {
    ...
    const before = fitSlots(saved?.slots) ?? [];
    const after = fitSlots(slots) ?? [];
    for (let i = 0; i < after.length; i++) {
      const was = before[i] == null ? null : String(before[i]);
      const now = after[i] == null ? null : String(after[i]);
      if (was === now) continue;
      // Named, so the message says which slot rather than making somebody
      // hunt for the one that is frozen.
      const nameFor = (id) => pool.find((x) => String(x.player_id) === id)?.name ?? 'That player';
      if (was && locked.has(was)) {
        throw new Error(`${nameFor(was)} has already played — that slot is locked.`);
      }
      if (now && locked.has(now)) {
        throw new Error(`${nameFor(now)} has already played — you cannot add him now.`);
      }
    }
  }
  ...
  try {
    ...
    // No transactions on the HTTP driver, so a buy-in taken a moment ago has
    // to be handed back by hand if the seat was never written.
    if (charged > 0) {
      await sql`
        insert into point_ledger (bettor, season, amount, reason, note)
        values (${slug}, ${contest.season}, ${charged}, 'refund',
                ${'Lobby buy-in returned -- entry failed'})`;
    }
    throw e;
  }
 ...
 * now.
 *
 * REVEALED PER PLAYER, not per lineup. A player shows once HIS OWN game has
 * kicked off; the rest of that lineup stays hidden until theirs do. Revealing
 * the whole eight once any one of them started meant somebody who rostered a
 * Thursday player had his entire Sunday team on the board from Thursday night
 * -- which is precisely the copying the hiding rule exists to stop, and with
 * three days to act on it.
 *
 * Also per LINEUP rather than per contest, for when to show a score at all. A
 * contest only leaves 'open' in the Tuesday cron, and the same run settles it
 * a moment later, so gating on contest status meant this page showed nothing
 * all weekend and then jumped straight to final.
 *
 * What a hidden slot gives away is deliberately nothing: no name, no position,
 * no salary. Even a position would narrow the guess, and salary would narrow
 * it further.
 */
export async function weekResults(season, week, { live = true } = {}) {
  const contests = await sql`
    ...
    where c.season = ${season} and c.week = ${week}
      and c.status in ('open', 'locked', 'settled')
  ...
  // Who has kicked off, once for the whole week. An open contest reveals a
  // lineup only when one of its players is in here.
  ...
  for (const c of contests) {
    ...
    const scored = field
      .map((f) => {
        // A finished contest has nothing left to protect: show everything.
        const settled = c.status !== 'open';
        // Anything at all to show? Before a single one of his players has
        // kicked off there is not even a partial score worth posting.
        const anyStarted = lineupLocked(f.slots, started);
        if (!settled && !anyStarted) {
          // Name and nothing else. Not even salary spent: the lobby page
          // already withholds that for the same reason, and a cap-maxed number
          // beside an empty row still says something about who is in it.
          const { slots: _hidden, salary_used: _spent, ...bare } = f;
          return { ...bare, hidden: true, live: null, players: [], playedOf: 0 };
        }

        const players = (f.slots ?? []).map((id, i) => {
          const key = String(id);
          // THE RULE: this player is visible once HIS game has started.
          const open = settled || started.has(key);
          if (!open) {
            return {
              slot: LINEUP[i],
              id: null,
        ...
        // `slots` carries every player id including the hidden ones, so it is
        // dropped rather than spread: the whole point is that those ids do not
        // leave the server.
        const { slots: _withheld, ...rest } = f;
        return {
        ...rest,
        hidden: false,
        // A settled contest keeps the number it settled at; a live one is
        // scored from the players who have actually played, so the running
        // total never counts points nobody can see yet.
        live:
          f.points != null
            ? f.points
            : players.reduce((sum, p) => sum + (p.hidden ? 0 : p.points), 0),
        // How much of the lineup is on the board, so the page can say
        // "3 of 8 played" rather than leaving a half-score unexplained.
        playedOf: players.filter((p) => !p.hidden).length,
        players,
        };
      })
      // Revealed lineups by score; the hidden ones after them, by name.
      .sort((a, b) => (b.live ?? -1) - (a.live ?? -1) || a.display_name.localeCompare(b.display_name));
...
export async function settleContest(contestId, { points = null } = {}) {
  ...
  if (c.kind === 'weekly') {
    ...
    //
    // Written under its OWN reason. These used to go in as 'trophies', and
    // point_ledger_trophies_once is unique on (bettor, season, week) for that
    // reason -- so the moment the cron had granted somebody's trophy points
    // for the week, their daily fantasy payout hit the index, `on conflict do
    // nothing` swallowed it, and `paid` reported it anyway. Nearly everyone
    // earns some trophy points every week, so nearly nobody was ever paid.
    const byPlace = {};
    ...
    for (const [p, group] of Object.entries(byPlace)) {
      ...
      for (const r of group) {
        ...
        await sql`
          ...
          values (${r.bettor}, ${c.season}, ${c.week}, ${each}, 'daily',
...
export async function saveDraft({ slug, contestId, slots }) {
  ...
  const clean = fitSlots(slots).map((id) =>
...
export async function lineupFor(slug, contestId) {
  const [entry] = await sql`
    select slots from dfs_entries where contest_id = ${contestId} and bettor = ${slug}`;
  if (entry) return { slots: fitSlots(entry.slots), entered: true };

  const [draft] = await sql`
    select slots from dfs_drafts where contest_id = ${contestId} and bettor = ${slug}`;
  return draft ? { slots: fitSlots(draft.slots), entered: false } : { slots: null, entered: false };
}
