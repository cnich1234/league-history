...
async function gameState(season, week) {
  ...
  // Market rows are left out entirely. 'trade' is what somebody spent on
  // shares and 'dividend' is what their shares paid, so either one, named,
  // tells the league who holds what -- and even its ABSENCE does ("Mike B
  // has no market activity" is a statement about his position). The first
  // draft of the week 2 recap wrote exactly that before this was caught.
  const MARKET_REASONS = new Set(['trade', 'dividend']);
  const earned = {};
  for (const r of pointRows) {
    if (MARKET_REASONS.has(r.reason)) continue;
