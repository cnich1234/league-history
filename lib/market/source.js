 ...
 * any game is on, a quarter hour otherwise.
 */
async function liveBoard(now) {
  ...
  const wrote = await recordTick(rows, now, { gap: anyLive ? TICK_EVERY_MS : QUIET_EVERY_MS }).catch(
    () => false,
  );
  ...
  if (wrote) {
    ...
    fills = await fillOrders(prices, { season: ctx.season, now }).catch((e) => ({ error: e.message }));
  }
