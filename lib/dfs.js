 ...
 * now. A lineup is shown once it is LOCKED -- once one of its players has
 * kicked off -- and withheld before that, or the last person in could copy
 * the best team on the board.
 *
 * Per LINEUP, not per contest. A contest only leaves 'open' in the Tuesday
 * cron, and the same run settles it a moment later, so gating on contest
 * status meant this page showed nothing all weekend and then jumped straight
 * to final. Somebody whose players have all started has nothing left to hide;
 * somebody rostering only the late game is still editing and stays hidden.
 */
export async function weekResults(season, week, { live = true } = {}) {
  ...
  for (const c of contests) {
    ...
    const scored = field
      .map((f) => {
        const revealed = c.status !== 'open' || lineupLocked(f.slots, started);
        // Not locked yet: the name and nothing else. No slots, no score --
        // even a total would say something about who is in it.
        if (!revealed) {
          return { ...f, slots: null, hidden: true, live: null, players: [] };
        }
        return {
        ...f,
        hidden: false,
