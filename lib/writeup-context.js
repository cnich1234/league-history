 ...
 * context never said who actually started Ja'Marr Chase. `playerOwners` and
 * `marketHoldings` both carry the owning manager for exactly that reason.
 */
...
/** Points, holdings and what the Market is doing. */
export async function bookState(season) {
  const db = sql();
  const [points, holdings, ps] = await Promise.all([
    db`select bettor, sum(amount)::int as points from point_ledger
       where season = ${season} group by bettor order by points desc`,
    db`select owner, player_id, shares from market_holdings
       where season = ${season} and shares > 0`,
    players(),
  ]);
  const owners = await playerOwnersBySeason();

  return {
    points: points.map((r) => ({ manager: r.bettor, points: r.points })),
    marketHoldings: holdings.map((h) => ({
      buyer: h.owner,
      shares: h.shares,
      player: ps[h.player_id]?.full_name ?? String(h.player_id),
      // The distinction the first draft got wrong: owning shares in a player
      // is not the same as having him on your fantasy roster.
      actuallyRosteredBy: owners[String(h.player_id)] ?? 'nobody in this league',
    })),
  };
}

async function playerOwnersBySeason() {
  const byRoster = await teams();
  const out = {};
  for (const t of Object.values(byRoster)) {
    for (const pid of t.players) out[pid] = t.manager;
  }
  return out;
}
