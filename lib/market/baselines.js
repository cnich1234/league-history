...
import { payDividends } from './trading.js';
...
/**
 * When this week actually began, as a timestamp.
 *
 * The roll is the only moment that matters to a price: every premium is
 * rewritten, every dividend is paid, and the whole board reprices at once. It
 * is not a wall-clock time -- it happens on the first tick after Sleeper flips
 * the week, which can be any hour of a Tuesday morning -- so the honest answer
 * is when the baselines were written, not when we guess they were.
 *
 * Null before a week has rolled, which is week 1 and nothing else. Callers
 * fall back to a rolling window.
 */
export async function weekStart(season, week) {
  const key = `${season}-${week}`;
  const hit = rollCache.get(key);
  if (hit && Date.now() - hit.at < TTL) return hit.ms;
  const [row] = await sql`
    select extract(epoch from min(created_at)) * 1000 as ms
    from market_baselines where season = ${season} and week = ${week}`;
  const ms = row?.ms == null ? null : Number(row.ms);
  rollCache.set(key, { at: Date.now(), ms });
  return ms;
}
...
export async function rollWeek({ season, fromWeek, toWeek, projFrom, actualFrom, premiumFrom }) {
  ...
  // The finished week's dividends, on the holdings as they stand at the roll.
  // Once per owner per week whatever happens; a failure here must not undo
  // the roll, which has already been written.
  let dividends = null;
  try {
    dividends = await payDividends({ season, week: fromWeek, actualFrom });
  } catch (e) {
    dividends = { error: e.message };
  }
  return { rolled: out.length, fromWeek, toWeek, dividends };
}
