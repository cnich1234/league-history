...
import { battleCounts, priceBattles, battleNoun, battleQuestion, FLEX_FROM } from './showdown.js';
...
export async function buildWeek(sql, season, week, { dryRun = false } = {}) {
  ...
  for (const pair of Object.values(byMatchup)) {
    ...
    // Position battles: QB, RB, WR and TE, plus a FLEX battle for whoever is
    // left over. Each side counts the same number of players, its best scorers
    // at the position (lib/showdown.js), and the lines come from simulating
    // the week and scoring every draw exactly as settlement will.
    {
      const { counts, flex } = battleCounts(slots);
      const battles = [
        ...['QB', 'RB', 'WR', 'TE'].filter((pos) => counts[pos]).map((position) => ({ position, counts })),
        ...(flex ? [{ position: 'FLEX', counts }] : []),
      ];
      const sideOf = (m) =>
        lineupOf(m)
          .filter((id) => counts[positionOf(id)])
          .map((id) => ({ id, position: positionOf(id), projection: projectPlayer(id) }));
      const homeSide = sideOf(a);
      const awaySide = sideOf(b);
      const priced = priceBattles(homeSide, awaySide, battles, {
        seed: `${season}:${week}:${a.roster_id}-${b.roster_id}`,
      });

      battles.forEach(({ position }, i) => {
        const price = priced[i];
        // A battle needs both sides to field someone, or it can only ever void.
        if (!price) return;
        const favIsHome = price.favouriteSide === 'home';
        const favTeam = favIsHome ? home : away;
        const dogTeam = favIsHome ? away : home;
        const { line } = price;
        const noun = battleNoun(position, counts);
        const odds = twoWayOdds(price.pCover);
        // Whoever might end up counted: every starter at the position, since
        // "best two" is only known at the end -- and for the flex, every
        // RB, WR and TE on both sides.
        const involved = [...homeSide, ...awaySide].filter((p) =>
          position === 'FLEX' ? FLEX_FROM.includes(p.position) : p.position === position,
        );

        createMarket({
          kind: 'showdown',
          title: `${favTeam.team} ${noun} -${line} vs ${dogTeam.team} ${noun}`,
          subtitle: battleQuestion(position, counts, favTeam.team, dogTeam.team, line),
          meta: {
            position,
            counts,
            homeRoster: a.roster_id,
            ...
            homeSlug: home.slug,
            awaySlug: away.slug,
            favouriteSide: price.favouriteSide,
            favouriteSlug: favTeam.slug,
            underdogSlug: dogTeam.slug,
            spread: line,
          },
          // A battle depends only on the players who might count, so it locks
          // on their kickoffs -- not on the matchup's, which is dragged early
          // by whichever player happens to play first. A QB battle where
          // neither quarterback plays Thursday has no business closing Thursday.
          locksAt: lockTimeFor(
            involved.map((p) => nflTeams[p.id]).filter(Boolean),
            gameDates,
            matchupLock,
          ),
          options: [
            { key: 'cover', label: `${favTeam.team} ${noun} -${line}`, odds: odds.home },
            { key: 'nocover', label: `${dogTeam.team} ${noun} +${line}`, odds: odds.away },
          ],
        });
      });
    }
