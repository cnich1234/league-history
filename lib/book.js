...
// Pure data, no database -- safe to import at the top rather than lazily like
// the rest of shop.js.
import { byKind, BOOSTS } from './boosts.js';
import { redundantLegs } from './implies.js';

/** Kinds that count as a hit on a bet, read from the catalogue. */
 ...
 *
 * The two totals are counted in SEPARATE subqueries, never by joining both
 * tables at once. Joining ledger and bets to the same bettor multiplies them
 * together -- every ledger row is repeated once per bet and every bet once per
 * ledger row -- so the aggregates came out scaled by the other table's row
 * count. It read as a plausible number rather than an obvious error: a bank of
 * $234.92 against 13 ledger rows and 8 bets showed as $1,879.36 and 39W/65L,
 * exactly 8x the money and 3x the record. Each bettor was inflated by a
 * different factor, so the standings were also in the wrong ORDER.
 */
export async function getBanks() {
  return sql`
    select b.slug,
           b.display_name,
           coalesce((
             select sum(l.amount_cents) filter (where l.week is null)
             from ledger l where l.bettor = b.slug
           ), 0)::bigint as bank_cents,
           coalesce((
             select count(*) filter (where bt.status = 'won')
             from bets bt where bt.bettor = b.slug
           ), 0)::int as wins,
           coalesce((
             select count(*) filter (where bt.status = 'lost')
             from bets bt where bt.bettor = b.slug
           ), 0)::int as losses,
           coalesce((
             select count(*) filter (where bt.status = 'pending')
             from bets bt where bt.bettor = b.slug
           ), 0)::int as pending
    from bettors b
...
/**
 * The week people are actually playing: the latest with open markets, or the
 * latest built if every week has closed.
 *
 * BOOK_WEEK is set in no environment, so everything that read it fell through
 * to week 1 and stayed there all season. The board and The Action were fixed;
 * My Boosts, the target picker, the shop and bounty routes and the standings
 * page were not -- so Ghost, Big Week and the curse were being declared for
 * week 1 while the league bet week 2, and did nothing. Five pages had grown
 * their own copy of this. There is one now.
 */
export async function currentWeek(season) {
...
export async function attackableBets(season, week = null) {
  return sql`
    select b.id,
           ...
           -- Attacks only. This counted every boost but Insurance, so a bet
           -- carrying Better Price, Half Again, Mirror or a Receipt read as
           -- "Hit" on The Action and was excluded from the bounty picker -- a
           -- 15-point Better Price was the cheapest shield in the shop.
           (select count(*)::int from boosts bo
              where bo.target_bet_id = b.id and bo.kind = any(${ATTACK_KINDS})) as attacked,
           -- Whether the bet can still be COPIED: its market is open. Says
           -- nothing about the pick -- a locked market's bets are already on
           -- The Floor. Attacks do not need this; they land after lock.
           coalesce((select m.status = 'open' from markets m where m.id = b.market_id), false)
             as rideable
    from bets b
    join bettors t on t.slug = b.bettor
...
/**
 * Game phase for each of a set of bets -- 'open' | 'live' | 'locked' |
 * 'closed'. The markets are read here, on the server, and only the WORD
 * leaves: nothing about which market or which side reaches the page.
 */
export async function phasesForBets(betIds) {
  if (!betIds?.length) return {};
  const ids = betIds.map(Number);
  const straight = await sql`
    select b.id, m.kind, m.live, m.meta, m.season, m.week
    from bets b join markets m on m.id = b.market_id
    where b.id = any(${ids})`;
  const legs = await sql`
    select l.bet_id, m.kind, m.live, m.meta, m.season, m.week
    from parlay_legs l join markets m on m.id = l.market_id
    where l.bet_id = any(${ids})
    order by l.bet_id`;

  const rows = straight.map((r) => ({
    id: Number(r.id),
    market: { kind: r.kind, live: r.live, meta: r.meta, season: r.season, week: r.week },
  }));
  const byParlay = {};
  for (const l of legs) {
    (byParlay[l.bet_id] ??= []).push({
      kind: l.kind, live: l.live, meta: l.meta, season: l.season, week: l.week,
    });
  }
  for (const [id, list] of Object.entries(byParlay)) rows.push({ id: Number(id), legs: list });

  const { betPhases } = await import('./live.js');
  return betPhases(rows);
}

/**
 * How these pending bets WOULD settle, keyed by bet id.
 *
 * For The Action, which shows bets whose games have finished but whose
 * settlement has not run -- the cron is on Tuesday and the last game ends
 * Monday night. Read-only: it projects from the same scores and the same
 * resolver settlement uses, and writes nothing.
 *
 * A bet whose result cannot be called yet is simply absent from the result.
 */
export async function standingForBets(betIds) {
  if (!betIds?.length) return {};
  const ids = betIds.map(Number);
  const straight = await sql`
    select b.id, b.option_key, m.kind, m.meta, m.season, m.week
    from bets b join markets m on m.id = b.market_id
    where b.id = any(${ids})`;
  const legs = await sql`
    select l.bet_id, l.option_key, m.kind, m.meta, m.season, m.week
    from parlay_legs l join markets m on m.id = l.market_id
    where l.bet_id = any(${ids})
    order by l.bet_id`;
  ...
  const rows = straight.map((r) => ({
    ...
    market: { kind: r.kind, meta: r.meta, season: r.season, week: r.week },
  }));
  const byParlay = {};
  for (const l of legs) {
    (byParlay[l.bet_id] ??= []).push({
      optionKey: l.option_key,
      market: { kind: l.kind, meta: l.meta, season: l.season, week: l.week },
    });
  }
  for (const [id, list] of Object.entries(byParlay)) rows.push({ id: Number(id), legs: list });

  const [{ standingResults }, { scoringInputs }] = await Promise.all([
    import('./standing.js'),
    import('./cron.js'),
  ]);
  return standingResults(rows, scoringInputs);
}

/** One bet's phase. Same classifier, one round-trip. */
export async function phaseOfBet(betId) {
  const phases = await phasesForBets([betId]);
  return phases[Number(betId)] ?? 'open';
}
...
export async function placeBet({
  ...
  if (pastLock && market.live) {
    // The model runs even when a locked price is in hand. A lock freezes the
    // NUMBER; it does not keep a decided market open or lift the cap on a
    // near-decided one -- both of those are about certainty, and thirty
    // minutes is long enough for a game to end.
    ...
    if (lockedBoostId == null) odds = priced[optionKey];
    ...
    // and tight enough to catch a scoring play landing mid-tap. A locked
    // price cannot move, so there is nothing to check.
    if (expectedOdds != null && lockedBoostId == null) {
  ...
  // A market in one of your parlays can also be bet straight, and a market
  // you bet straight can be a parlay leg, in as many parlays as you like.
  // The old exclusivity stopped nothing worth stopping -- every leg carries
  // the house cut -- and read to the league as "you cannot reuse a bet".

  // A field market -- the weekly specials, where every manager is an option --
  // takes as many straight bets as you like, one per manager.
  //
  // One-bet-per-market was written when every market had two sides, where a
  // second bet could only ever be more money on a side you already held. On a
  // ten-horse field it blocks something completely different: backing a
  // DIFFERENT manager, an outcome that cannot win alongside your first. Three
  // reasons it is safe here and not elsewhere:
  //
  //   - The money is already governed. Every placement checks the week's
  //     spendable below, so two $100 bets cost exactly what one $200 does.
  //   - The outcomes are mutually exclusive. At most one of them can pay.
  //   - The vig makes it self-limiting. fieldOdds prices every option with a
  //     4.5% margin, so covering more of the field is strictly worse value.
  //     Buying the whole board is a guaranteed loss, which needs no rule.
  //
  // The database still enforces one bet per SIDE (bets_one_per_side), so the
  // thing the original rule actually protected against -- stacking the same
  // outcome to dodge a limit -- remains impossible.
  const isField = market.kind === 'special';

  // A voided bet -- undone, or reversed by the commissioner -- never
  // happened as far as this rule is concerned. Counting it left the market
  // locked for its owner after the stake had already gone back.
  //
  // On a field market the same test runs against the OPTION rather than the
  // market. bets_one_per_side would catch it regardless, but a raw constraint
  // violation is not a sentence anyone should be shown.
  const [existing] = isField
    ? await sql`
        select 1 from bets
        where bettor = ${slug} and market_id = ${marketId}
          and option_key = ${optionKey} and status <> 'void'`
    : await sql`
    select 1 from bets where bettor = ${slug} and market_id = ${marketId} and status <> 'void'`;
  if (existing && isField) {
    throw new Error('You already have a bet on that manager.');
  }
  if (existing) {
  ...
  //
  // Only a bet of minStakeDollars or more wears it off. Without that floor the
  // counter is a $1 punt, which discharges a 5-point attack for a cent -- and
  // once anyone works that out they would open every week with one, which is
  // both unbeatable and dull. A bet under the floor is left completely alone:
  // not doubled, and it does not burn the boost.
  const { pendingSlowPlay } = await import('./shop.js');
  const slowPending = await pendingSlowPlay(slug, market.week);
  const slowFloorCents = (byKind['slow-play']?.minStakeDollars ?? 0) * 100;
  const slowed = slowPending && stakeCents >= slowFloorCents ? slowPending : null;
...
export async function placeParlay({ slug, legs, stakeCents }) {
  ...
  const markets = await sql`
    select id, status, locks_at, live, kind, title, meta, season, week
  ...
  // A leg that is guaranteed by another leg is not a second question, and a
  // parlay only pays what it pays because its legs are separate questions.
  //
  // Chris built Devin +4.5, Devin by 20.5+ and Devin by 30.5+ and the slip
  // quoted +3278. All three win exactly when the hardest one does, so the
  // honest price was +397 -- the book would have paid 8x for nothing. Stacking
  // all four margin markets on one roster reaches 35x.
  //
  // The strictest leg survives; the ones it already contains are refused by
  // name, so the message says which leg to drop and why.
  const nested = redundantLegs(
    legs.map((l) => ({
      marketId: Number(l.marketId),
      optionKey: l.optionKey,
      market: marketById[Number(l.marketId)],
    })),
  );
  if (nested.length) {
    const { redundant, impliedBy } = nested[0];
    const titleOf = (id) => marketById[id]?.title ?? 'that market';
    throw new Error(
      `${titleOf(impliedBy.marketId)} already covers ${titleOf(redundant.marketId)} — ` +
        `if one wins the other must too, so the parlay would be paid twice for one result. ` +
        `Drop one of them.`,
    );
  }
  ...
  // A parlay is a bet, so Slow Play applies to it too. Without this a slowed
  // manager could place a $500 parlay at face value and the boost would still
  // be sitting there afterwards -- a hole straight through the stake floor.
  const { pendingSlowPlay: parlayPending } = await import('./shop.js');
  const slowPendingParlay = await parlayPending(slug, parlayWeek);
  const parlayFloorCents = (byKind['slow-play']?.minStakeDollars ?? 0) * 100;
  const parlaySlowed =
    slowPendingParlay && stakeCents >= parlayFloorCents ? slowPendingParlay : null;
  const parlayCostCents = parlaySlowed ? stakeCents * 2 : stakeCents;

  const parlaySpendable = await weeklyBalance(slug, parlayWeek);
  if (parlaySpendable < parlayCostCents) {
    throw new Error(
      parlaySlowed
        ? `You have been slowed: this parlay costs ${formatMoney(parlayCostCents)} to ` +
          `place. You have ${formatMoney(parlaySpendable)}.`
        : `Not enough left this week. You have ${formatMoney(parlaySpendable)}.`,
    );
  }
  ...
  await sql`
    insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
    values (${slug}, ${-parlayCostCents}, 'stake', ${bet.id},
            ${`${priced.length}-leg parlay${parlaySlowed ? ' (slowed -- double cost)' : ''}`},
            ${parlayWeek})`;

  if (parlaySlowed) {
    const { spendSlowPlay } = await import('./shop.js');
    await spendSlowPlay(parlaySlowed, bet.id);
  }
...
export async function settleParlays() {
  ...
  for (const bet of pending) {
    ...
    // to forget.
    const { boostsForBets: parlayBoosts, thievesFor, tithesFor } = await import('./shop.js');
    ...
    // Grand Theft and Cut of the Action, exactly as a straight bet takes them.
    // This path passed neither, so a thief paid 27 points and the victim kept
    // every cent -- on the kind of bet the league says it will mostly place.
    const [thieves, tithes] = await Promise.all([thievesFor([bet.id]), tithesFor([bet.id])]);
    await sql`
 ...
 */
async function bookPayout(sql, { bet, payout, refund, week, stolenBy = null, tithes = [] }) {
  ...
  if (stolenBy && !refund) {
    // A crowd-funded steal has no single thief. The payout goes to the people
    // who paid for the attack, in proportion to what each put in -- the
    // rounding remainder to the largest share, so the parts add back up to the
    // payout exactly.
    if (stolenBy.bountyId != null) {
      const { splitBountyPayout } = await import('./shop.js');
      const shares = await splitBountyPayout(stolenBy.bountyId, payout);
      for (const share of shares) {
        await sql`
          insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
          values (${share.slug}, ${share.cents}, 'payout', ${bet.id},
                  'Stolen bet (bounty share)', null)`;
      }
      return;
    }
    await sql`
      insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
      values (${stolenBy.slug}, ${payout}, 'payout', ${bet.id}, 'Stolen bet', null)`;
    return;
  }
  ...
  // Cut of the Action: somebody takes a share of the PROFIT. Not of the
  // payout -- that would claw back part of their stake and could leave a
  // winning bet a net loss, which is far more than a 5-point boost should do.
  let owed = 0;
  for (const t of tithes) {
    const cut = Math.floor(profit * t.share);
    if (cut <= 0) continue;
    owed += cut;

    if (t.bountyId != null) {
      // Crowd-funded: the share is split again between whoever paid for it.
      const { splitBountyPayout } = await import('./shop.js');
      for (const part of await splitBountyPayout(t.bountyId, cut)) {
        await sql`
          insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
          values (${part.slug}, ${part.cents}, 'payout', ${bet.id},
                  'Cut of the action (bounty share)', null)`;
      }
    } else {
      await sql`
        insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
        values (${t.slug}, ${cut}, 'payout', ${bet.id}, 'Cut of the action', null)`;
    }
  }

  // Never more than the profit, however many people are taking a slice: the
  // book cannot pay out more than the bet won.
  const kept = Math.max(0, profit - Math.min(owed, profit));
  if (kept > 0) {
    await sql`
      insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
      values (${bet.bettor}, ${kept}, 'payout', ${bet.id}, 'Winnings banked', null)`;
  }
}
...
export async function settleMarket(marketId, winningOption) {
  ...
  const { thievesFor, tithesFor } = await import('./shop.js');
  const thieves = await thievesFor(bets.map((b) => b.id));
  // Who is owed a share of each winner. Unlike a theft this leaves the bet
  // with its owner -- it only splits the profit.
  const tithes = await tithesFor(bets.map((b) => b.id));
  ...
  // A bounty still raising for any of these bets can never land now. Refund
  // its backers here rather than holding their points until Tuesday's cull.
  const { cancelBountiesOnBet } = await import('./shop.js');
  for (const bet of bets) await cancelBountiesOnBet(Number(bet.id));
...
export async function settledBets(season, limit = 200) {
  ...
  //
  // `week` comes from the market, and a parlay has no market -- so every parlay
  // came back with a null week and the results page crashed grouping them:
  // Object.keys turned the bucket into the STRING "null", Number("null") is
  // NaN, and byWeek[NaN] is undefined. A parlay's week is the week its stake
  // was taken from, which is what the ledger records and what a returned stake
  // is paid back into (see stakeWeekOf). Its legs can span weeks; the stake
  // only ever came out of one.
  return sql`
    select b.id, b.bettor, b.option_key, b.stake_cents, b.odds, b.status,
           b.payout_cents, b.settled_at, b.is_parlay,
           m.title, m.subtitle, m.kind, m.winning_option,
           coalesce(m.week, (
             select l.week from ledger l
             where l.bet_id = b.id and l.reason = 'stake' and l.week is not null
             limit 1
           )) as week,
...
export async function settledSummary(season) {
  return sql`
    select b.bettor, t.display_name,
           ...
           count(*) filter (where b.status in ('push', 'void', 'cashed'))::int as refunded,
...
export async function lockDueMarkets(finishedRosters = [], kickedOffTeams = [], scope = null) {
  ...
  // Everything else non-live whose deadline has simply passed.
  //
  // Showdowns and blowout spreads carry homeRoster/awayRoster rather than an
  // nflTeam, and no `special` flag -- so the clause above could never match
  // them and they stayed open indefinitely. Twenty-four of them were still
  // taking bets three days after kickoff.
  //
  // For a non-live market `locks_at` IS the deadline: it was set from the
  // earliest kickoff the market depends on, so past it there is nothing left
  // to wait for. Live markets are excluded because their timestamp means
  // something else entirely -- when pricing switched to the live model, not a
  // closing time -- and they close on finished rosters below.
  // Scoped by KIND, and narrowly.
  //
  // A prop whose player we cannot place in a game is deliberately left alone:
  // closing it on a timestamp would be guessing at kickoff, which is the
  // mistake this whole function exists to avoid. Same for h2h and total, which
  // are always live in practice and close on finished rosters below.
  //
  // Showdowns and blowout spreads are the two that had no route to closing at
  // all. They carry homeRoster/awayRoster rather than an nflTeam and no
  // `special` flag, so the kickoff clause above could never match them -- and
  // for these `locks_at` genuinely IS the deadline, set from the matchup's
  // earliest kickoff, with no per-player question left to answer.
  //
  // Scoped, and only when a scope is given. Unscoped it would sweep every
  // season in the table at once -- which is how an earlier unscoped reopen
  // reached eight real production markets during a test run. A caller that
  // does not say which week it means does not get a bulk write.
  const overdue =
    season != null && week != null
      ? await sql`
          update markets set status = 'locked'
          where status = 'open' and live = false and locks_at <= now()
            and kind in ('showdown', 'spread')
            and season = ${season} and week = ${week}
          returning id, title`
      : [];
  if (overdue.length) {
    console.warn(`Locked ${overdue.length} market(s) past their deadline.`);
  }
  ...
  if (!rosters.length) return [...plain, ...overdue];
