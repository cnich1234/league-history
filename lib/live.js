...
import { expectedLineup, replacementTable, isRisky, DEFAULT_SLOTS } from './lineup.js';
...
async function loadProjections(season, week) {
  ...
  for (const r of await res.json()) {
    ...
    const position = r.player?.position ?? r.position;
    if (position) positionById[r.player_id] = position;
    const injury = r.player?.injury_status ?? null;
    if (injury) injuryById[r.player_id] = injury;
  }
  return {
    projections: byId,
    teamOfPlayer: teamById,
    positionOfPlayer: positionById,
    injuryOfPlayer: injuryById,
  };
}

/** The league's starting slots, fetched once per process. */
let slotsCache = null;
async function leagueSlots() {
  if (slotsCache) return slotsCache;
  try {
    const league = await api(`/league/${LEAGUE_ID}`);
    slotsCache = Array.isArray(league?.roster_positions) && league.roster_positions.length
      ? league.roster_positions
      : DEFAULT_SLOTS;
  } catch {
    slotsCache = DEFAULT_SLOTS;
  }
  return slotsCache;
}
...
/** A side is final when its starters have nothing left to come -- and it has starters. */
export function sideFinal(side) {
  return side.remaining === 0 && (side.players ?? 1) > 0;
}
 ...
 */
export function sides(matchup, projections, gameByTeam, teamOfPlayer, lineupOpts = null) {
  const points = matchup?.starters_points ?? [];
  const gameOf = (id) => {
    const team = teamOfPlayer[id];
    return team ? gameByTeam[team] : null;
  };

  // Priced on the lineup the roster can be expected to field, not the one
  // currently set -- see lib/lineup.js for why. Without slot and position
  // data (old callers, tests) it falls back to the starters as set.
  const entries = lineupOpts
    ? expectedLineup(matchup, {
        slots: lineupOpts.slots,
        positionOf: (id) => lineupOpts.positionOf?.[id] ?? null,
        projectionOf: (id) => {
          const team = teamOfPlayer[id];
          if (team != null && gameOf(id) == null) return 0; // bye
          return projections[id] ?? 0;
        },
        kickedOff: (id) => {
          const g = gameOf(id);
          return g ? hasKickedOff(g) : false;
        },
        unavailable: lineupOpts.unavailable ?? new Set(),
        doubtful: (id) => isRisky(lineupOpts.injuryOf?.[id]),
        replacement: lineupOpts.replacement ?? {},
      }).filter((e) => e.id || e.replacement > 0)
    : (matchup?.starters ?? [])
        .map((id, i) => ({ id, index: i, locked: true }))
        .filter((e) => e.id && e.id !== '0');

  let scored = 0;
  let remaining = 0;
  for (const { id, index, replacement } of entries) {
    // A slot nobody on the roster can fill: worth a waiver pickup, all to come.
    if (!id) {
      remaining += Number(replacement) || 0;
      continue;
    }
    if (index != null) scored += Number(points[index] ?? 0);

    // A starter on a bye has no game this week, so nothing of theirs is still
    // to come. Byes begin in week 5 and hit four or five teams at a time, so
    // this is not an edge case for most of the season.
    //
    // Two bugs met here and compounded. `projections[id] ?? 9` invented 9
    // points for anyone missing from the projections map, and
    // `fractionRemaining(undefined)` returns 1 because an unknown game is
    // treated as not yet kicked off. A bye starter therefore contributed a
    // full 9 points of phantom "still to come" that could never arrive.
    //
    // The damage was not cosmetic: a lineup 10 points down late in the week
    // with two bye starters priced at 73% to win instead of 10%, and the
    // market stayed open because the model thought 18 points were coming.
    const team = teamOfPlayer[id];
    const game = team ? gameByTeam[team] : null;
    const onBye = team != null && game == null;
    if (onBye) continue;

    // An unknown player -- not in the projections map and not identifiably on
    // a bye -- still gets a placeholder, since dropping them would understate
    // a lineup that does have someone playing.
    const projection = projections[id] ?? 9;
    remaining += projection * fractionRemaining(game);
  }
  return {
    ...
    // How many starters the lineup actually has. A lineup nobody has set yet
    // has none, and "nothing left to come" must not read as "all done".
...
export async function liveMatchups(season, week) {
  const [users, rosters, matchups, proj, gameByTeam, slots] = await Promise.all([
  ...
  const { projections, teamOfPlayer, positionOfPlayer, injuryOfPlayer } = proj;
  // Players who cannot be started this week whatever the bench says.
  const benched = {};
  const rostered = new Set();
  for (const r of rosters) {
    benched[r.roster_id] = new Set([...(r.reserve ?? []), ...(r.taxi ?? [])].map(String));
    for (const id of r.players ?? []) rostered.add(String(id));
  }
  // What a slot nobody can fill is worth: the best of the waiver wire.
  const replacement = replacementTable({
    ids: Object.keys(projections),
    rostered,
    positionOf: (id) => positionOfPlayer[id] ?? null,
    projectionOf: (id) => {
      const team = teamOfPlayer[id];
      if (team != null && !gameByTeam[team]) return 0; // bye
      return projections[id] ?? 0;
    },
  });
  const lineupOptsFor = (m) => ({
    slots,
    positionOf: positionOfPlayer,
    injuryOf: injuryOfPlayer,
    unavailable: benched[m.roster_id] ?? new Set(),
    replacement,
  });
  ...
  for (const pair of Object.values(byMatchup)) {
    ...
    const [a, b] = pair;
    const home = sides(a, projections, gameByTeam, teamOfPlayer, lineupOptsFor(a));
    const away = sides(b, projections, gameByTeam, teamOfPlayer, lineupOptsFor(b));
    ...
    //
    // With one exception: a lineup with NO starters has nothing to come
    // because nobody has set it, not because its games are over. A board
    // opened on a Friday for the following week found three managers who had
    // not touched their lineups yet, called their sides final, and locked
    // their team totals before a single game had been scheduled.
    const homeFinal = sideFinal(home);
    const awayFinal = sideFinal(away);
...
/**
 * Where a bet stands in its game, for The Action and for every attack.
 *
 *   open    nothing it depends on has kicked off. Attackable, and the ONLY
 *           phase Ride Along is allowed in -- copying a bet whose game is
 *           underway is copying with hindsight.
 *   live    underway, and the market reprices as it goes: h2h, spread, total.
 *   locked  underway, and the market does not reprice -- a prop, a position
 *           battle, a special, a blowout line. Bets are closed; the result
 *           is not. Attackable.
 *   closed  every game it depends on is final. Nothing can be done to it. An
 *           attack on a known result is not a gamble, and until this existed
 *           one was allowed all the way to Tuesday's settlement.
 *
 * Decided from what Sleeper reports about the games, never from a market's
 * status or its locks_at. A 'locked' market can be hours before kickoff
 * (locks_at is midnight) or a finished game waiting to be settled, and the
 * status cannot tell those apart.
 *
 * `state` is liveMatchups' output plus the per-team game map; see betPhases.
 */
export function phaseFor(market, { matchups, games }) {
  const meta = market.meta ?? {};
  let started = false;
  let finished = false;

  if (market.kind === 'prop') {
    const g = meta.nflTeam ? games[meta.nflTeam] : null;
    started = hasKickedOff(g);
    finished = g ? fractionRemaining(g) === 0 : false;
  } else if (meta.special) {
    // Every lineup in the league is in it.
    const all = Object.values(matchups);
    started = all.some((x) => x.started);
    finished = all.length > 0 && all.every((x) => x.home.final && x.away.final);
  } else {
    const m = stateForMarket(market, matchups);
    if (m) {
      if (meta.homeRoster != null && meta.awayRoster != null) {
        started = m.started;
        finished = m.home.final && m.away.final;
      } else {
        const side = meta.rosterId === m.homeRoster ? m.home : m.away;
        started = m.started || side.scored > 0;
        finished = side.final;
      }
    }
  }
  ...
  return market.live ? 'live' : 'locked';
}

/**
 * A parlay's phase from its legs. Over only when every leg is; untouched
 * only when every leg is; in between it is live if any leg reprices.
 */
export function parlayPhase(legPhases, legLive) {
  if (!legPhases.length) return 'open';
  if (legPhases.every((p) => p === 'closed')) return 'closed';
  if (legPhases.every((p) => p === 'open')) return 'open';
  return legPhases.some((p, i) => p === 'live' && legLive[i]) ? 'live' : 'locked';
}

/**
 * Phases for a set of bets, one Sleeper round-trip per (season, week).
 *
 * `rows` is [{ id, market, legs }] where market is { kind, live, meta,
 * season, week } for a straight bet and legs is an array of the same for a
 * parlay. Returns { [betId]: phase }.
 */
export async function betPhases(rows) {
  const states = {};
  const stateFor = async (season, week) => {
    const key = `${season}-${week}`;
    if (!states[key]) {
      const [live, games] = await Promise.all([
        liveMatchups(season, week),
        loadGames(season, week),
      ]);
      states[key] = { matchups: live.matchups, games };
    }
    return states[key];
  };
  ...
  for (const r of rows) {
    if (r.market) {
      out[r.id] = phaseFor(r.market, await stateFor(r.market.season, r.market.week));
      continue;
    }
    const phases = [];
    const lives = [];
    for (const leg of r.legs ?? []) {
      phases.push(phaseFor(leg, await stateFor(leg.season, leg.week)));
      lives.push(Boolean(leg.live));
    }
    out[r.id] = parlayPhase(phases, lives);
  }
  return out;
}
...
export function livePrice(market, state, { margin = LIVE_MARGIN } = {}) {
  ...
  if (market.kind === 'h2h') {
    ...
    // A blowout is a three-outcome field keyed by roster id, not a two-sided
    // spread. Nothing here describes it, so it is not quoted live -- which
    // suspends it once games start rather than pricing it as something else.
