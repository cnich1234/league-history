...
/**
 * The most a set of players could have scored in these slots, after the fact.
 *
 * `players` is [{ position, points }] -- everyone who was actually available,
 * starters and bench alike. Slots are filled most-restrictive first (QB, RB,
 * ... then FLEX, then SUPER_FLEX), each with the best remaining player who
 * fits. Because every flex slot accepts a superset of the positions below it,
 * filling in that order is optimal: a flex slot never takes a player a
 * dedicated slot needed.
 *
 * Exists for the lineup trophies. They used to compare a bench player only
 * against starters at his own position, so a tight end on the bench was never
 * weighed against a receiver in the FLEX. In week 2 of 2026 that handed Chris R
 * Perfect Lineup and Manager of the Week with Mark Andrews (10.9) benched
 * while receivers scoring 6.5 and 6.8 started -- the FLEX could have taken him,
 * and the lineup was 4.40 short of its best.
 */
export function bestLineupPoints(players, slots = DEFAULT_SLOTS) {
  const rank = (slot) => (slot === 'SUPER_FLEX' ? 2 : slot === 'FLEX' ? 1 : 0);
  const order = startingSlotsOf(slots).slice().sort((a, b) => rank(a) - rank(b));
  const pool = (players ?? [])
    .filter((p) => p && p.position)
    .map((p) => ({ position: p.position, points: Number(p.points ?? 0) }))
    .sort((a, b) => b.points - a.points);
  let total = 0;
  for (const slot of order) {
    const i = pool.findIndex((p) => slotFits(slot, p.position));
    if (i === -1) continue;
    total += pool[i].points;
    pool.splice(i, 1);
  }
