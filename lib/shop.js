...
export async function useBoostOnBet({ slug, boostId, betId, season = null }) {
  ...
  // Bounties no longer pay a claimer. A collective bounty buys the attack
  // outright and fires itself, so there is nothing for a bought attack to
  // collect -- see fireBounty.
  return { ...row, reflected, bounties: [] };
}
...
export async function contributeToBounty({ slug, season, bountyId, points }) {
  ...
  const now = await bountyTotal(bountyId);
  let fired = null;
  if (now >= cost) fired = await fireBounty(bountyId);

  return { raised: now, cost, funded: now >= cost, contributed: take, fired };
}
...
export async function fireBounty(bountyId) {
  ...
  const [boost] = await sql`
    ...
    values (${b.poster}, ${b.season}, ${b.weapon}, ${Number(b.cost_points)},
            ...
            ${JSON.stringify({ bounty: String(bountyId), week: Number(b.week) })}::jsonb)
    returning id`;
  ...
  return { fired: true, boostId: String(boost.id), weapon: def.name };
}
