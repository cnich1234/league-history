...
/** How many orders are waiting for a tick. */
export async function pendingCount(season) {
  const [row] = await sql`
    select count(*)::int as n from market_orders where season = ${season} and status = 'pending'`;
 ...
 *
 * ON THE CLOCKS. `placed_at` is written by the DATABASE (now() in the insert)
 * while `now` arrives from the CALLER, and the two machines do not agree: this
 * Neon instance measured a steady 1.2 seconds ahead of the app. A strict
 * `placed_at <= tickAt` therefore drops an order placed in the same second as
 * the tick, which is precisely the order somebody just tapped and is watching
 * for -- it sat until the next tick a minute later, looking broken.
 *
 * Five seconds of grace. Comfortably more than any drift seen between these
 * two machines, and still far short of the minimum gap between ticks, so the
 * rule that matters is untouched: an order cannot be filled by a tick recorded
 * BEFORE it was placed, which is what stops anyone trading on a touchdown the
 * feed has not published yet.
 */
const CLOCK_GRACE_MS = 5000;

export async function fillOrders(prices, { season, now = Date.now(), names = {} } = {}) {
  const label = (id) => names[id] ?? id;
  const tickAt = new Date(now);
  const cutoff = new Date(now + CLOCK_GRACE_MS);
  const pending = await sql`
    select id, owner, season, week, player_id, side, shares from market_orders
    where status = 'pending' and season = ${season} and placed_at <= ${cutoff}
  ...
  for (const o of pending) {
    ...
    const reject = async (why) => {
      ...
      out.rejected++;
      await notify(o.owner, {
        title: `❌ Order not filled: ${o.side} ${o.shares} × ${label(o.player_id)}`,
        body: why,
    ...
    if (o.side === 'buy') {
      ...
      out.filled++;
      await notify(o.owner, {
        title: `✅ Bought ${o.shares} × ${label(o.player_id)} at ${price}`,
        body: `${cost} points. Holdings are on your portfolio.`,
        url: '/market/portfolio',
        tag: `fill-${o.id}`,
      }).catch(() => 0);
    } else {
      ...
      out.filled++;
      await notify(o.owner, {
        title: `✅ Sold ${o.shares} × ${label(o.player_id)} at ${price}`,
        body: `+${proceeds} points to spend.`,
        url: '/market/portfolio',
        tag: `fill-${o.id}`,
      }).catch(() => 0);
    }
...
export async function payDividends({ season, week, actualFrom }) {
  ...
  for (const [owner, total] of perOwner) {
    ...
    if (rows.length) {
      paid++;
      await notify(owner, {
        title: `💸 Week ${week} dividends: +${whole} points`,
        body: 'Paid into your points. The breakdown is on your portfolio.',
        url: '/market/portfolio',
        tag: `dividend-${week}`,
      }).catch(() => 0);
    }
