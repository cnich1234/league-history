import { battleScore } from './showdown.js';
 ...
 * manager nothing and does not belong in the comparison. Which starters count
 * is lib/showdown.js's battleScore: the same number from each side (the best
 * two RBs, say), with whoever is left over forming the FLEX battle. Markets
 * from before that rule carry no `counts` and are scored the old way, every
 * starter at the position. The handicap is applied to the underdog exactly as
 * a spread is, and lines are half-points so a push is impossible by
 * construction.
 *
 * Voids rather than guesses if either side has nobody to count: a lineup with
 * no TE cannot lose a TE showdown, because there was never a bet to win.
 */
function resolveShowdown(market, { starterRosters, positionOf }) {
  ...
  const started = { [homeRoster]: [], [awayRoster]: [] };
  for (const [playerId, entry] of Object.entries(starterRosters)) {
    if (!(entry.rosterId in started)) continue;
    started[entry.rosterId].push({ position: positionOf(playerId), points: entry.points });
  }

  const home = battleScore(started[homeRoster], meta);
  const away = battleScore(started[awayRoster], meta);
  // null means nobody to count, which is different from scoring zero.
  if (home == null || away == null) return 'void';
