...
export async function getBanks() {
  return sql`select slug, display_name, bank_cents from banks order by bank_cents desc, display_name`;
}
 ...
 */
export async function attackableBets(season) {
  return sql`
    select b.id,
           b.bettor,
           t.display_name as bettor_name,
...
export async function placeBet({ slug, marketId, optionKey, stakeCents, expectedOdds = null }) {
  ...
  if (spendable < stakeCents) {
    throw new Error(`Not enough left this week. You have ${formatMoney(spendable)}.`);
  }
  ...
  await sql`
    ...
    values (${slug}, ${-stakeCents}, 'stake', ${bet.id}, 'Bet placed', ${market.week})`;
...
async function bookPayout(sql, { bet, payout, refund, week, stolenBy = null }) {
  ...
  // Grand Theft: the winnings go to the thief instead. The victim still gets
  // their stake back -- they paid it, and the boost takes the upside rather
  // than the whole wager. Anything else would be a fine rather than a theft.
  if (stolenBy && !refund) {
    const profit = payout - stake;
    if (stake > 0) {
      await sql`
        insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
        values (${bet.bettor}, ${stake}, 'stake-return', ${bet.id},
                'Stake returned -- winnings stolen', ${week ?? null})`;
    }
    if (profit > 0) {
      await sql`
        insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
        values (${stolenBy}, ${profit}, 'payout', ${bet.id}, 'Stolen winnings', null)`;
    }
    return;
  }
  ...
  // Stake back into the week it came from, so it can be bet again.
  if (stake > 0) {
    await sql`
      insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
      values (${bet.bettor}, ${stake}, 'stake-return', ${bet.id}, 'Stake returned on a win',
              ${week ?? null})`;
  }

  // Profit to the bank: week null, which is what makes it permanent.
  const profit = payout - stake;
  if (profit > 0) {
    await sql`
      insert into ledger (bettor, amount_cents, reason, bet_id, note, week)
