...
// `status` is what decides open vs closed now -- a past locks_at no longer
// means shut, since a prop's posted time is midnight on the morning of its game
// and a live market trades right through its own.
const mk = (id, kind, meta, locks, status = 'open') => ({
  id,
  kind,
  meta,
  title: `${kind} ${id}`,
...
const all = [
  mk(1, 'h2h', { homeRoster: 1, awayRoster: 2 }, PAST, 'locked'),
  mk(2, 'spread', { homeRoster: 1, awayRoster: 2 }, PAST, 'locked'),
  mk(3, 'total', { rosterId: 1 }, PAST, 'locked'),
  // A second total, still open, so the sort has two to order rather than one.
  mk(10, 'total', { rosterId: 2 }, FUTURE),
...
check('regardless of when they lock', card.marketCount, 6);
check('including the closed h2h', card.markets.h2h.length, 1);
check('and the open props', card.markets.prop.length, 2);
check('open count excludes locked markets', card.openCount, 3);
...
check('the locked matchup still appears', stillThere?.marketCount, 3);
check('holding only what is open', stillThere?.markets.h2h.length, 0);

console.log('\nsorting');
check(
  'open markets come before closed ones within a kind',
  card.markets.total.map((m) => m.status),
  ['open', 'locked'],
);
const mixed = groupByMatchup(
  [
    mk(8, 'prop', { rosterId: 1 }, PAST, 'locked'),
    mk(9, 'prop', { rosterId: 1 }, FUTURE),
    ...all,
  ],
  all,
);
const props = mixed.find((g) => g.key === '1-2').markets.prop;
check('open props first', props[0].status, 'open');
check('closed props last', props[props.length - 1].status, 'locked');
