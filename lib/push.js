 ...
 *
 * Every suite runs against a sentinel season in the 99xx range, against the
 * same database and with real bettor slugs -- which is what keeps them honest.
 * But `notify` takes a slug, not a season, so a test that filled an order sent
 * "Bought 2 x p1 at 12" to a real person's lock screen. Chris got three of them
 * in one afternoon from `npm run test:markettrade`, which is how this was found.
 *
 * The guard is here rather than in the twenty-two suites that can reach a
 * notification, because one place that cannot be forgotten beats twenty-two
 * that can. Set PUSH_SILENT=1 to silence a run explicitly; the test scripts do.
 */
function silenced() {
  if (process.env.PUSH_SILENT === '1') return true;
  // A belt-and-braces catch for anything run by `node scripts/*.test.mjs`
  // directly, without the npm script that sets the variable.
  const entry = process.argv[1] ?? '';
  return /\.test\.mjs$/.test(entry);
}
...
export async function notify(slug, payload) {
  // Before anything else, and before any database work: a test run sends
  // nothing. notifyMany and notifyAll both funnel through here, so this one
  // check covers every send in the app.
