...
export async function useBoostOnBet({ slug, boostId, betId, season = null }) {
  ...
  // A bounty pays out here rather than at settlement: the thing it asked for
  // has happened, and waiting would mean nobody knows they earned it. A
  // REFLECTED attack collects nothing -- it landed on the attacker, not the
  // person somebody wanted hit.
  let collected = [];
  if (def.attack && !reflected && targetSeason != null) {
    const [wk] = await sql`select week from markets where id = ${bet.market_id}`;
    if (wk?.week != null) {
      collected = await claimBounties({
        attacker: slug,
        victim: bet.bettor,
        weapon: boost.kind,
        season: targetSeason,
  ...
  return { ...row, reflected, bounties: collected };
}
...
export async function switcheroo({ slug, boostId, betId }) {
  ...
  // Where the switch actually lands. A straight bet has one market; a parlay
  // has several, so a LEG is picked at random first -- the attacker cannot see
  // the picks, so they cannot aim at the leg that matters.
  ...
  if (victim.is_parlay) {
    // Only legs that are still live can move. A leg whose market has already
    // settled has a result; rewriting it would change history rather than the
    // bet's future.
  ...
  if (victim.is_parlay) {
    ...
    // odds are recomputed from the legs at settlement, so there is nothing
  ...
  let collected = [];
  if (!reflected) {
    // A parlay has no market_id of its own, so take the week from the leg that
    // actually moved.
    const [wk] = await sql`select week from markets where id = ${legMarketId}`;
    if (wk?.week != null) {
      collected = await claimBounties({
        attacker: slug,
        victim: bet.bettor,
...
export async function slowPlay({ slug, boostId, target, week }) {
  ...
  const boostSeason = boost?.season ?? null;
  if (!boost) throw new Error('No such boost.');
  if (boost.owner !== slug) throw new Error('That is not yours.');
  if (boost.used_at) throw new Error('That boost has already been used.');
  ...
  const collected = await claimBounties({
    attacker: slug,
    victim: target,
    weapon: 'slow-play',
    season: boostSeason,
  ...
  return { target, week, bounties: collected };
}
...
/**
 * Puts points on someone's head for a SPECIFIC attack.
 *
 * Not a boost -- it buys nothing for the poster, it pays someone else to act.
 * That is the point: it turns attacking into a market, and it is the only thing
 * in the app that makes one manager's move worth money to another.
 *
 * The reward is escrowed immediately. Posting a bounty you cannot cover would
 * let someone advertise ten points, watch a claim land, and have nothing to pay
 * with -- so the points leave the poster's balance the moment it goes up.
 */
export async function postBounty({ slug, season, week, target, weapon, rewardPoints }) {
  const reward = Math.round(Number(rewardPoints));
  if (!Number.isFinite(reward) || reward < 1) throw new Error('A bounty needs a reward.');
  ...
  const points = await getPoints(slug, season);
  if (points < reward) {
    throw new Error(`Not enough points. That bounty costs ${reward}, you have ${points}.`);
  }

  const [clash] = await sql`
    select id from bounties
    where season = ${season} and week = ${week} and target = ${target}
      and weapon = ${weapon} and claimed_by is null`;
  if (clash) throw new Error(`There is already a ${def.name} bounty on them this week.`);

  // Escrowed now, refunded if it expires unclaimed.
  await sql`
    insert into point_ledger (bettor, season, amount, reason, note)
    values (${slug}, ${season}, ${-reward}, 'purchase',
            ${'Bounty on ' + victim.display_name})`;

  const [row] = await sql`
    insert into bounties (season, week, poster, target, weapon, reward_points)
    values (${season}, ${week}, ${slug}, ${target}, ${weapon}, ${reward})
  ...
  return {
    ...
    weapon: def.name,
    reward,
    // The alert copy, written once here so an in-app banner and a future push
    // notification say exactly the same thing.
    alert: `BOUNTY ALERT: A bounty has been placed on ${victim.display_name.toUpperCase()} ` +
      `ATTACK: ${def.name} REWARD: ${reward} Points`,
  };
}

/** Open bounties for a week, newest first. */
export async function openBounties(season, week) {
  return sql`
    select b.id, b.target, b.weapon, b.reward_points, b.posted_at,
           t.display_name as target_name,
           p.display_name as poster_name
    from bounties b
    join bettors t on t.slug = b.target
    join bettors p on p.slug = b.poster
    where b.season = ${season} and b.week = ${week} and b.claimed_by is null
    order by b.reward_points desc, b.posted_at desc`;
}

/**
 * Pays out any bounty this attack just satisfied.
 *
 * Called after an attack lands. Matching on (target, weapon) means hitting the
 * right person the wrong way earns nothing -- which is what makes a named
 * weapon a real constraint rather than decoration.
 */
export async function claimBounties({ attacker, victim, weapon, season, week, boostId }) {
  const rows = await sql`
    update bounties
    set claimed_by = ${attacker}, claimed_at = now(), claim_boost_id = ${boostId ?? null}
    where season = ${season} and week = ${week}
      and target = ${victim} and weapon = ${weapon}
      and claimed_by is null
      -- Posting a bounty and collecting it yourself would be a way to move
      -- points nowhere while looking busy.
      and poster <> ${attacker}
    returning id, reward_points, poster`;

  for (const r of rows) {
    await sql`
      insert into point_ledger (bettor, season, amount, reason, note)
      values (${attacker}, ${season}, ${r.reward_points}, 'refund',
              ${'Bounty collected'})`;
  }
  return rows.map((r) => ({ id: String(r.id), reward: r.reward_points }));
}

/**
 * Refunds bounties nobody collected.
 *
 * Run when a week closes. An unclaimed bounty is not forfeit -- nobody did the
 * thing that was asked for, so the points go home.
 */
export async function expireBounties(season, week) {
  const rows = await sql`
    delete from bounties
    where season = ${season} and week = ${week} and claimed_by is null
    returning poster, reward_points`;
  for (const r of rows) {
    await sql`
      insert into point_ledger (bettor, season, amount, reason, note)
      values (${r.poster}, ${season}, ${r.reward_points}, 'refund', 'Bounty expired unclaimed')`;
  }
  return rows.length;
}
...
export async function marketEffects(season, week) {
  ...
  const weekly = await sql`
    ...
    where b.season = ${season}
      ...
      and b.used_at is not null
      and b.kind in ('boost-week', 'week-curse')`;
  ...
  return {
    ...
    weekly: weekly.map((r) => ({
      kind: r.kind,
      // A curse names its victim; a self-boost names its owner.
      who: r.kind === 'week-curse' ? r.target_name : r.owner_name,
      by: r.kind === 'week-curse' ? r.owner_name : null,
