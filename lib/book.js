 ...
 *
 * A live market is bettable past its posted lock -- that is the whole point of
 * it -- so `locks_at > now()` alone would drop exactly the markets that are
 * most active. Whether a live market is *currently* quotable depends on game
 * state and is decided by `shouldSuspend` at pricing time, not here.
 */
 ...
 *
 * A market is safe to reveal only when it is genuinely finished with: either it
 * is not live and its lock has passed, or it is live and no longer open.
 *
 * The live half of that rule leans on `lockDueMarkets` having run. That is now
 * the case, but the failure mode if it ever stops is worth naming: markets stay
 * `open`, and these bets stay hidden. Hiding too much is the safe direction --
 * the alternative was exposing a live stake to someone still choosing a side.
 */
export async function visibleBets(season, week) {
  return sql`
    ...
    where m.season = ${season} and m.week = ${week}
      and m.locks_at <= now()
      and (m.live = false or m.status <> 'open')
 ...
 *
 * Two different closing rules, and conflating them was a real bug: a plain
 * market shuts at its posted time, but a live market is *designed* to trade
 * past that time and only shuts when its game is over. Locking live markets on
 * `locks_at` would have closed them the moment they became interesting.
 *
 * `finishedRosters` is the set of roster ids whose games are final, from
 * Sleeper. Passing an empty set locks only the non-live markets, which is the
 * safe direction: a market left open is still governed by `shouldSuspend` at
 * pricing time, so nothing can be bet at a decided price either way.
 *
 * `kickedOffTeams` is the set of NFL teams whose games have actually started.
 * A market is not locked until its player has taken the field, because
 * `locks_at` is midnight Arizona on the morning of the game -- fine for a 1pm
 * Sunday kickoff, but for a Thursday night game it closes betting some
 * eighteen hours early. Locking on that alone shut a prop for a game that had
 * not begun, and put the bettor's position on The Floor while everyone else
 * could still see it coming.
 */
export async function lockDueMarkets(finishedRosters = [], kickedOffTeams = null) {
  // `null` means the caller could not determine kickoffs; fall back to the
  // posted time rather than leaving everything open forever. An empty set is
  // different and meaningful: nothing has kicked off, so nothing plain locks.
  const teams = kickedOffTeams == null ? null : [...kickedOffTeams].filter(Boolean);
  ...
  const plain =
    teams == null
      ...
      : await sql`
          update markets set status = 'locked'
          where status = 'open' and live = false and locks_at <= now()
            and (meta->>'nflTeam' is null or meta->>'nflTeam' = any(${teams}))
          returning id, title`;
  ...
  const live = await sql`
    update markets set status = 'locked'
    where status = 'open' and live = true and locks_at <= now()
      and (
